		...
		private static Visibility GetVisibilityMode(object parameter) {
			...
			if (parameter != null) {
				...
				if (parameter is Visibility) {
					...
					try {
						mode =
							(Visibility) Enum.Parse(typeof (Visibility), parameter.ToString(), true);
					}
					catch (FormatException e) {
						throw new FormatException(
							"Invalid Visibility specified as the ConverterParameter.  Use Visible or Collapsed.",
							e);
					}
				}
			}
